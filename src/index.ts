...
export interface ScriptInputs {
  ...
  'post-comment'?: string;
  'allowed-authors'?: string;
  'max-prs-per-day'?: string;
  'require-associated-issue'?: string;
...
export = async function runVerification({ github, context, core, inputs }: AsyncOptions): Promise<void> {
  ...
  const failCheck = (reasonMsg: string, logMsg: string) => {
  ...
  let workFlowRunsInspectable: boolean;
  ...
  if (String(inputs['require-associated-issue']).toLowerCase() === 'true') {
    try {
      ...
      const gqlResult = await github.graphql<GraphQLResponse>(query, {
        owner: context.repo.owner,
        repo: context.repo.repo,
        prNumber: prNumber
      });

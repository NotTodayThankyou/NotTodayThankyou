"use strict";
module.exports = async function runVerification({ github, context, core, inputs }) {
    ...
    // =========================================================================
    const maxPrs = parseInt(inputs['max-prs-per-day'] || '-1', 10);
    if (maxPrs >= 0) {
        try {
            ...
            if (searchResults.total_count > maxPrs) {
                failCheck(`❌ **PR Closed**: You have opened ${searchResults.total_count} PRs across GitHub in the last 24 hours, exceeding the daily threshold of ${maxPrs}.`, `Author '${author}' exceeded rate limit (${searchResults.total_count}/${maxPrs} PRs created in 24h).`);
            }
            else {
    ...
    if (String(inputs['require-associated-issue']).toLowerCase() === 'true') {
      ...
      `;
            ...
            if (closingIssuesCount === 0 && !hasTimelineIssueReference) {
                failCheck('❌ **PR Closed**: This repository requires pull requests to be linked to an associated issue.', 'No linked issue found via closingIssuesReferences or timelineItems cross-references.');
            }
            else {

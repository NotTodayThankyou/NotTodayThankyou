...
async function run() {
  try {
    ...
    // Retrieve token automatically from inputs or environment
    const token = process.env.GITHUB_TOKEN || core.getInput('github-token');
    if (!token) {
      core.setFailed('A GitHub Token is required to initialize Octokit instance.');
      return;
    }
    ...
    // Retrieve Inputs
    const inputs = {
      'close-message': core.getInput('close-message'),
      'post-comment': core.getInput('post-comment'),
      'allowed-authors': core.getInput('allowed-authors'),
      'max-prs-per-day': core.getInput('max-prs-per-day'),
      'require-associated-issue': core.getInput('require-associated-issue')
    };
